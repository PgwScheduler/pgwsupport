...
export function ScheduleView({ store }) {
  ...
  return (
    <div>
            ...
            return (
              <React.Fragment key={week[0]}>
                {week.map((date) => {
                  ...
                  return (
                    <button
                      ...
                      className={
                        "min-h-[112px] border-b border-r border-hairline p-1.5 text-left align-top hover:bg-surface-overlay " +
                        (dim ? "bg-surface-page" : "")
                      }
                    >
                        ...
                        <span
                          className={
                            isToday
                              ...
                              : "text-xs font-medium " + (dim ? "text-content-muted" : "text-content-secondary")
                          }
                ...
                <button
                  ...
                  className="min-h-[112px] border-b border-hairline bg-surface-overlay p-2 text-right hover:bg-surface-overlay"
                >
                  <div className="flex items-center justify-end gap-1 text-sm font-bold text-content-primary">
                    {fmtHours(summary.total)}
                    <ChevronDown
                      className={"h-3.5 w-3.5 text-content-muted transition-transform " + (expanded ? "rotate-180" : "")}
                    />
                  </div>
                  <div className="text-[10px] uppercase tracking-wide text-content-muted">hrs</div>
                </button>

                {expanded && (
                  <div className="col-span-8 border-b border-hairline bg-surface-overlay px-4 py-3">
                    ...
                    {summary.employees.length === 0 ? (
                      <p className="text-sm text-content-muted">No shifts scheduled this week.</p>
                    ) : (
                      <div className="grid gap-x-8 gap-y-1 sm:grid-cols-2 lg:grid-cols-3">
                        {summary.employees.map((e) => (
                          <div key={e.id} className="flex items-center justify-between gap-3 text-sm">
                            <span className="truncate text-content-primary">{e.name}</span>
                            <span className="flex-shrink-0 font-medium text-content-secondary">{fmtHours(e.hours)} h</span>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      {loading && <p className="mt-3 text-sm text-content-muted">Loading…</p>}
