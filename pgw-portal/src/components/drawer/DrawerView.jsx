    ...
    <div className="space-y-5">
      ...
      {editingId && (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-warning-border bg-warning-tint px-4 py-2.5 text-sm text-warning">
          ...
          <button onClick={cancelEdit} className="text-xs font-medium text-warning underline-offset-2 hover:underline">Cancel</button>
        </div>
      )}
      ...
      <div>
        <h3 className="pgw-display mb-2 text-sm font-bold uppercase tracking-wide text-content-secondary">1 · Drawer Cash Count</h3>
      ...
      <div>
        <h3 className="pgw-display mb-2 text-sm font-bold uppercase tracking-wide text-content-secondary">2 · Cash to Deposit</h3>
        <Card className="p-4">
          ...
          <div className="mt-2 flex items-center justify-between rounded-lg px-3 py-2.5" style={{ backgroundColor: T.accentSoftBg }}>
            <div>
              ...
              <p className="text-[11px] text-content-muted">Put this amount in your deposit envelope</p>
            </div>
      ...
      <div>
        <h3 className="pgw-display mb-2 text-sm font-bold uppercase tracking-wide text-content-secondary">3 · Daily Sales Summary</h3>
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="p-4">
          ...
          <div className="space-y-4">
            <Card className="p-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className={"rounded-lg px-3 py-2 " + (t.overage > 0 ? "bg-success-tint" : "bg-surface-overlay")}>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-content-secondary">Cash Overage</p>
                  <p className={"pgw-display text-lg font-bold " + (t.overage > 0 ? "text-success" : "text-content-muted")}>{money(t.overage)}</p>
                </div>
                <div className={"rounded-lg px-3 py-2 " + (t.shortage < 0 ? "bg-danger-tint" : "bg-surface-overlay")}>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-content-secondary">Cash Shortage</p>
                  <p className={"pgw-display text-lg font-bold " + (t.shortage < 0 ? "text-danger" : "text-content-muted")}>{money(t.shortage)}</p>
                </div>
              </div>
              {t.overage > 0 && (
              ...
              <p className="mt-3 text-[11px] text-content-muted">Petty cash is already deducted from your deposit — no need to add it to the short/why box.</p>
            </Card>
      ...
      <div>
        <h3 className="pgw-display mb-2 text-sm font-bold uppercase tracking-wide text-content-secondary">4 · Invoices, POAs &amp; Payouts</h3>
        <div className="grid gap-4 lg:grid-cols-2">
          <MiniTable
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <h3 className="pgw-display text-sm font-bold uppercase tracking-wide text-content-secondary">Saved closeouts</h3>
          <div className="flex items-center gap-2">
            {canExportRange && (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-hairline">
                {saved.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="cursor-pointer odd:bg-surface-stripe hover:bg-surface-overlay" onClick={() => setViewing(r)}>
                      <td className="px-4 py-2.5 text-content-secondary">{r.business_date}</td>
                      <td className="px-4 py-2.5 font-medium text-content-primary">{money(rt.storeDeposit)}</td>
                      <td className={"px-4 py-2.5 font-semibold " + (rt.diff < 0 ? "text-danger" : rt.diff > 0 ? "text-success" : "text-content-muted")}>{money(rt.diff)}</td>
                      <td className="px-4 py-2.5 text-content-secondary">{money(rt.pettyTotal)}</td>
                      ...
                      {canDelete && (
                        <td className="px-4 py-2.5 text-right">
                          <button onClick={(ev) => { ev.stopPropagation(); deleteCloseout(r.id); }} className="text-content-muted hover:text-danger">
