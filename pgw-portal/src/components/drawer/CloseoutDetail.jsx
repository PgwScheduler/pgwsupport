...
export function CloseoutDetail({ record, store, onClose, onEdit }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-scrim p-4" onClick={onClose}>
      <div className="mx-auto max-w-5xl" onClick={(ev) => ev.stopPropagation()}>
        <Card className="overflow-hidden">
          <div className="sticky top-0 flex items-center justify-between gap-3 border-b border-hairline bg-surface-card px-5 py-3">
            <div>
              <h3 className="pgw-display text-base font-bold text-content-primary">Closeout — {record.business_date}</h3>
          ...
          <div className="space-y-5 p-5">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <div className="rounded-lg p-3" style={{ backgroundColor: T.accentSoftBg }}>
                <p className="text-[11px] uppercase tracking-wide text-content-secondary">Cash to Deposit</p>
              ...
              <div className="rounded-lg bg-surface-overlay p-3">
                <p className="text-[11px] uppercase tracking-wide text-content-secondary">Store Deposit to Bank</p>
                <p className="pgw-display text-lg font-bold text-content-primary">{money(t.storeDeposit)}</p>
              </div>
              <div className={"rounded-lg p-3 " + (t.diff < 0 ? "bg-danger-tint" : t.diff > 0 ? "bg-success-tint" : "bg-surface-overlay")}>
                <p className="text-[11px] uppercase tracking-wide text-content-secondary">Over / Short</p>
                <p className={"pgw-display text-lg font-bold " + (t.diff < 0 ? "text-danger" : t.diff > 0 ? "text-success" : "text-content-secondary")}>{money(t.diff)}</p>
              </div>
              <div className="rounded-lg bg-surface-overlay p-3">
                <p className="text-[11px] uppercase tracking-wide text-content-secondary">Petty Cash Used</p>
                <p className="pgw-display text-lg font-bold text-content-primary">{money(t.pettyTotal)}</p>
              </div>
            </div>

            {(e.overage_why || e.shortage_why) && (
              <Card className="p-4">
                {e.overage_why && <p className="text-sm text-content-secondary"><span className="font-semibold text-content-primary">Overage explanation: </span>{e.overage_why}</p>}
                {e.shortage_why && <p className="mt-1 text-sm text-content-secondary"><span className="font-semibold text-content-primary">Shortage explanation: </span>{e.shortage_why}</p>}
              </Card>
            )}

            <div>
              <h4 className="pgw-display mb-2 text-xs font-bold uppercase tracking-wide text-content-muted">Drawer Cash Count</h4>
            ...
            <div>
              <h4 className="pgw-display mb-2 text-xs font-bold uppercase tracking-wide text-content-muted">Daily Sales Summary</h4>
              <div className="grid gap-4 lg:grid-cols-2">
                <Card className="p-4">
            ...
            <div>
              <h4 className="pgw-display mb-2 text-xs font-bold uppercase tracking-wide text-content-muted">Invoices, POAs &amp; Payouts</h4>
              <div className="grid gap-4 lg:grid-cols-2">
