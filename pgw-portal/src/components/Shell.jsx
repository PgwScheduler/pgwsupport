...
export function Shell({ view, setView, children }) {
  ...
  return (
    <div className="pgw-root flex min-h-screen bg-slate-950 text-slate-100">
      {/* Sidebar */}
      <aside className="hidden w-60 flex-shrink-0 flex-col justify-between border-r border-slate-800 bg-slate-900 p-4 md:flex">
        <div>
            ...
            {[...NAV, ...(role === "master" ? MASTER_NAV : [])].map((n) => {
              ...
              return (
                <button
                  ...
                  className={"flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-sm font-medium " + (active ? "" : "text-slate-300 hover:bg-slate-800")}
                >
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-950 p-3">
          ...
          <p className="pgw-display mt-1 text-sm font-bold text-white">{scopeLabel(profile, stores.length)}</p>
        ...
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-slate-900 px-5 py-3">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden flex-col items-end sm:flex">
              ...
              <span className="text-xs text-slate-500">{ROLE_LABELS[role] ?? role}</span>
            </div>
