...
//   labor              = labor_sales + 0.5*adjustments        (Summary R36)
//   parts_and_supplies = parts_sales + supplies + 0.5*adjustments (Summary R37)
...
// Adjustments (the source sheet's Groupon column) split evenly between
// labor and parts.
//
...
// (tech_store_month); parts/tires/supplies/adjustments/discounts and the
// parts/tire costs come from daily_kpi. Effective Labor Rate uses the
// SAME adjustments-blended labor as Summary!R22:
//   ELR = (labor_sales + 0.5*adjustments) / flag_hours
...
export function computeGrossProfit(k, tech) {
  ...
  const adjustments = num(k?.adjustments);
  const discounts = num(k?.discounts); // stored signed (<= 0)
  const partsCost = num(k?.parts_cost);
  const tireCost = num(k?.tire_cost);

  const halfAdjustments = 0.5 * adjustments;

  const laborLine = laborSales + halfAdjustments; // R36
  const partsAndSupplies = parts + supplies + halfAdjustments; // R37
...
// Official store ELR — adjustments-blended labor over flag hours (Summary R22).
export function effectiveLaborRate(laborSales, adjustments, flagHours) {
  const flag = num(flagHours);
  if (flag === 0) return 0;
  return (num(laborSales) + 0.5 * num(adjustments)) / flag;
}
