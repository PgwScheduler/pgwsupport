...
export function useDirectory() {
  const [data, setData] = useState({ stores: [], contacts: [], coverage: [], districts: [], regions: [] });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const load = useCallback(async () => {
    setError(null);
    const [s, c, cv, d, r] = await Promise.all([
    ...
    const firstError = [s, c, cv, d, r].find((x) => x.error)?.error;
    if (firstError) {
      setError(firstError.message);
    } else {
      setData({ stores: s.data ?? [], contacts: c.data ?? [], coverage: cv.data ?? [], districts: d.data ?? [], regions: r.data ?? [] });
    }
    setLoading(false);
  }, []);
  ...
  return { ...data, loading, error, reload: load, updateStore, saveContact, setContactActive };
}
